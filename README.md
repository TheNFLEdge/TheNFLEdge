...
For an in-progress week, run the manually triggered `NFL In-Progress Score Refresh` workflow from the GitHub Actions tab. It runs only `npm run refresh:scores`, which updates completed score markers in `nfleTMP.htm` and root weekly pages while leaving the archive and rotation workflow untouched.
